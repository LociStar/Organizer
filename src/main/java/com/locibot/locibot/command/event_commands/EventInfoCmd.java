...
public class EventInfoCmd extends BaseCmd {
    ...
    public Mono<?> execute(Context context) {
        return DatabaseManager.getEvents().getInvitedDBEvent(context.getAuthorId(), context.getOptionAsString("name").orElseThrow()).flatMap(dbEvent ->
                context.getClient().getUserById(dbEvent.getOwner().getUId()).flatMap(owner -> {
                    Long time = dbEvent.getBean().getScheduledDate();
                    return context.createFollowupMessage("Loading").flatMap(Message::delete)
                            .then(context.createMessage(EmbedCreateSpec.builder()

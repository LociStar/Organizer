                            ...
                            DatabaseManager.getEvents().getAllEvents().forEach(dbEvent -> {
                                ...
                                if (epochSeconds != null) {
                                    ...
                                    if (zonedDateTime.toLocalDate().isEqual(LocalDate.now())
                                            ...
                                            && zonedDateTime.toLocalTime().isAfter(LocalTime.now().plus(1950, ChronoUnit.SECONDS))
                                            && zonedDateTime.toLocalTime().isBefore(LocalTime.now().plus(1650, ChronoUnit.SECONDS))) {
                                                ...
                                                gateway.getUserById(member.getId()).flatMap(user ->
                                                        user.getPrivateChannel().flatMap(privateChannel ->
                                                                privateChannel.createMessage(EmbedCreateSpec.builder()
                                                                        ...
                                                                        .footer(EmbedCreateFields.Footer.of("Author: " + dbEvent.getOwner().getEventName(), ""))
                                                                        .build())
                                                        )).subscribe();
                                        });
                                    }
